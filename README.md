...
|이름|느낀 점|
|-----|-----|
|[최범석](https://github.com/ausudu)|처음으로 로봇 팔을 다뤄보며, 이를 활용할 방안을 고민하는 과정이 무척 흥미로웠다. 특히, DART 스튜디오를 사용해 로봇 팔을 원하는 대로 움직이게 만들었을 때는 상당한 성취감을 느꼈다. 하지만 움직임을 코드로 작성하고 저장하는 과정은 예상보다 복잡했고, 많은 어려움을 겪었다. 이전에 파이썬 함수에 대해 학습하며 어려움을 느꼈던 경험이 떠오르며, 여전히 해당 부분에서 부족함이 발목을 잡고 있다는 생각이 들었다. 또한, 로봇을 어떤 방식으로 활용할 수 있을지 창의적인 아이디어를 구상하는 것 역시 쉽지 않았다. 다른 팀원들의 의견이나 타 조의 발표를 들으며 기발하고 흥미로운 아이디어들이 많이 나오는 것을 보며 감탄했지만, 내 머릿속에서 새로운 발상을 만들어 내는 일이 생각처럼 간단하지 않음을 실감했다. 이를 계기로 창의적인 사고와 기술적 역량 모두를 더 깊이 탐구하고 개선해야겠다는 생각을 하게 되었다.  |
...
|[김영수](https://github.com/youngsoo-kim-123)|DART 플랫폼을 이용해 로봇을 제어하고 시나리오를 구성하고, 그 시나리오를 구현할 수 있는 코드를 짜는 방법이 실습의 내용이였다. 엄밀히 따지면, 이번주 수업은 시나리오를 어떻게 짜는지, 그 시나리오에 맞춰 코드는 어떻게 하는지가 주제이며, 더 정확히는 '이미' 만들어진 로봇을 어디에 적용해야 할 지에 대한 실무능력을 배우는 단계였다. 수단은 로봇 소프트웨어지만, 주제는 시나리오와 관련 코딩을 짜는 능력이라 할 수 있겠다. 주제가 로봇 소프트웨어인 수업은 언제 들을 수 있는지 모르겠다.|